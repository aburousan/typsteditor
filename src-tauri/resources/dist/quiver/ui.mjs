...
class Panel {
    ...
    initialise(ui) {
        ...
        // Typst Editor: this is a Typst editor, so the LaTeX/tikz-cd renderer,
        // import and export controls are removed — diagrams are inserted as
        // fletcher (Typst) via the host's "Insert Diagram" button. Only a
        // shareable-link (URL) button and the macros field are kept.
        void renderer_select; void import_from_tikz; void export_to_latex; void export_to_typst;
        this.global = new DOM.Div({ class: "panel global" }).add(
            new DOM.Element("label").add("Share: ")
        ).add(
            // The shareable link button (permalink to re-open this diagram).
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- Typst Editor bridge: expose ui and answer export requests from the host.
    window.ui = ui;
    window.addEventListener("message", (e) => {
        if (!e.data || !e.data.quiverRequestExport) return;
        try {
            const { data } = ui.quiver.export("fletcher", ui.settings, ui.options(), ui.definitions());
            (e.source || window.parent).postMessage({ quiverExport: data }, "*");
        } catch (err) {
            (e.source || window.parent).postMessage({ quiverExportError: String(err) }, "*");
        }
    });
    ui.initialise();
